...
pub const CURRENT_BLOCK_HEIGHT: u64 = 845000;
...
pub fn add_utxo(utxos: Vec<Utxo>, new_utxo: Utxo) -> Vec<Utxo> {
    ...
    //todo!()
    let mut mutable_utxos = utxos;
    mutable_utxos.push(new_utxo);
    mutable_utxos
}
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    //todo!()
    if height < 0 {
        (false, String::from("Height is negative"))
    } else if height > 1_000_000 {
        (false, String::from("Height is unrealistic"))
    } else {
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    match hex::decode(version_hex) {
        ...
        Err(_) => Err(String::from("Hex decode error")),
    }
}
